#!/usr/bin/python3
"""
Module 101-queens.py contains an algorithm that resolves the N-Queen puzzle
using backtracking
"""


def isSafe(m_queen, nqueen):
    """ A Method that determines if the queens can kill each other
    Args:
        m_queen: an array that has the queens positions
        nqueen: queen number
    Returns:
...
def print_result(m_queen, nqueen):
    """ A Method that prints the list with the Queens positions
    Args:
    ...
    for i in range(nqueen):
        positions_list.append([i, m_queen[i]])

    print(positions_list)


def Queen(m_queen, nqueen):
    """ Recursive function that runs the Backtracking algorithm
    Args:
        m_queen: array that has the queens positions
        nqueen: queen number
    """
